...
server <- function(input, output) {
  ...
  meanInt <- renderPlot({
    ...
    plot(1, type = "n",
         ...
         ylab = "Mean Intensity", xlab = input$timeUnits)
    points(x = 1:length(mIntTime())*input$frameRate, y = mIntTime(), pch = 19)
  })
  ...
  output$BvIHist <- renderPlot({
    out <- ggplot(data = data.frame(Intensity = as.vector(intenseDat()),
           ...
           mapping = aes(x = Intensity, y = Brightness)) +
      stat_bin2d(bins = input$BvIbin) +
      scale_x_continuous(limits = range(c(input$BvIxMin, input$BvIxMax))) +
      scale_y_continuous(limits = range(c(input$BvIyMin, input$BvIyMax))) +
      theme_classic()
    if(input$IBHistROI == 1){
      out <- out + geom_rect(aes(xmin = input$BvIROIxMin, xmax = input$BvIROIxMax, ymin = input$BvIROIyMin, ymax = input$BvIROIyMax), color = "red", alpha = 0)
    }
    out
  })
  ...
  gatedVals <- reactive({
    d <- brightDat()
    subY <- which(d >= input$BvIROIyMin & d <= input$BvIROIyMax)
    d <- intenseDat()
    subX <- which(d >= input$BvIROIxMin & d <= input$BvIROIxMax)
    intersect(subY, subX)
  })
  ...
  output$gatedBrightness <- renderPlot({
    ...
    e <- intenseDat()
    subX <- e > input$BvIROIxMin & e < input$BvIROIxMax & !is.na(d)
    gatedVals <- subY & subX
    im <- as.cimg(rd()[,,,1])
    im <- im/max(im)
    #plot(imager::colorise(
    #  im,
    #  gatedVals,
    #  "firebrick2",
    #  alpha = .5
