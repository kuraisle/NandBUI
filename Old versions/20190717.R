...
server <- function(input, output) {
  ...
  output$gateTable <- renderDT(gt(), selection = "none", editable = TRUE)
  ...
  grt <- reactive({
    ...
    proc <- apply(gt(), 1, gateThat)
    allPix <- gateThat(apply(gt(), 2, max))
    data.frame(absolute = proc, percent = proc/allPix*100)
  })
  
  output$gatedResults <- renderDT(grt(), selection = "none", editable = F)
