...
server <- function(input, output) {
  ...
  observeEvent(
    input$addGate,{
      nGates <<- nGates + 1
      output[[paste0("gatePage", nGates)]] <- renderUI({
        list(
          fluidPage(
            fluidRow(
              column(
                width = 2,
                numericInput(paste0("nGateMin", nGates), "Number minimum", 0)
              ),
              column(
                width = 2,
                numericInput(paste0("nGateMax", nGates), "Number maximum", max(numberDat(), na.rm = T))
              ),
              column(
                width = 2,
                numericInput(paste0("bGateMin", nGates), "Brightness minimum", 0)
              ),
              column(
                width = 2,
                numericInput(paste0("bGateMax", nGates), "Brightness maximum", max(brightDat(), na.rm = T))
              ),
              column(
                width = 2,
                numericInput(paste0("iGateMin", nGates), "Intensity minimum", 0)
              ),
              column(
                width = 2,
                numericInput(paste0("iGateMax", nGates), "Intensity maximum", max(intenseDat(), na.rm = T))
              )
            )
          )
        )
